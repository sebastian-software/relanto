  ...
  return withDomainErrorJson(request, async () => {
    ...
    if (auth.kind === "token") {
      if (
        ...
        throw new Response("Token cannot pause this job", { status: 403 });
      }

      return Response.json({
        job: mailerApi.pauseJob(auth.token.applicationAdminId, auth.token.kind, params.jobId),
        ok: true,
      });
    }

    return Response.json({
      job: mailerApi.pauseJob(auth.principalId, "systemAdmin", params.jobId),
      ok: true,
    });
  });
}
