...
Checksum- und Digest-Aufzeichnung, Trivy-Positivkontrolle, Secret-Scan und ein nicht veröffentlichender Digest-Erhaltungsnachweis liegen gemeinsam in `scripts/verify-release-archive.sh`. Für diesen Nachweis kopiert `skopeo copy --preserve-digests --digestfile` das Archiv in ein temporäres lokales OCI-Layout. Ausgewählter Manifest-Digest, Digestfile und Layout-Digest müssen identisch sein. Der Release-Workflow ruft das Skript vor Laden, Smoke-Test und Veröffentlichung auf. Der `container`-Job in `.github/workflows/ci.yml` führt dasselbe Skript bereits vor dem Merge aus: mit einem einmal gebauten `linux/amd64`-OCI-Archiv, ohne Registry-Login, ohne Push und ohne Docker-Socket. Ein zweites Image baut der Job nicht. Anschließend lädt `scripts/load-release-archive.sh` genau dieses unveränderte Archiv per `docker load`. Lehnt der Image-Store das OCI-Archiv ab, wie der klassische Image-Store des GitHub-Runners, wandelt der digest-gepinnte Skopeo-Container es dateibasiert in ein temporäres Docker-Archiv mit derselben Config um und `docker load` lädt dieses; beide Wege kommen ohne Socket-Mount und ohne `docker-daemon:`-Transport aus. Danach prüft das Skript die Identität des geladenen Tags: Die Image-ID muss beim OCI-Archiv dem Manifest-Digest (containerd-Image-Store) oder dem Config-Digest (klassischer Image-Store) des Archivs entsprechen, beim umgewandelten Docker-Archiv dem Config-Digest, und die RootFS-Layer müssen in derselben Reihenfolge den `rootfs.diff_ids` der Archiv-Config gleichen. Gegen dieses geladene Image laufen der Image-Verifier, der Trivy-Schwachstellenscan und beide Smoke-Tests (Standard und Operator-Assets-Fixture); danach wird die Archiv-Checksum erneut geprüft. Der Release-Workflow nutzt denselben Ladeweg: `scripts/load-release-archive.sh` lädt sein Archiv unter der beim Build gesetzten Referenz `ghcr.io/sebastian-software/relanto:<release-tag>` in den lokalen Image-Store, ebenfalls ohne Socket-Mount und ohne `docker-daemon:`-Transport, und prüft dieselbe Identität. Image-Verifier und beide Smoke-Tests laufen gegen genau diese lokal geladene Referenz, danach wird die Archiv-Checksum erneut geprüft. Erst anschließend veröffentlicht `skopeo copy` das unveränderte Archiv; aus dem lokalen Image-Store wird nichts veröffentlicht. Ein grüner Pull-Request-Lauf belegt so Positivkontrolle, Secret-Scan, Digest-Erhaltung, Laden und Smoke-Test desselben Archivs vor dem Merge; die Nachweise des realen Releases ersetzt er nicht.
